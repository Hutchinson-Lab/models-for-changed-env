# Plot all results on UCI datasets
...
from .plot_descriptions import general_plots_metadata, characteristics_plots_metadata, metrics_plots_metadata

output_table_dir = './experiments/UCI_MLR/tables/'

output_plot_main_dir = './experiments/UCI_MLR/plots/'

output_plot_general_dir = './experiments/UCI_MLR/plots/general/'
output_plot_general_c_dir = './experiments/UCI_MLR/plots/general/cost/'
output_plot_general_c_n_dir = './experiments/UCI_MLR/plots/general/cost/norm/'
output_plot_general_c_e_dir = './experiments/UCI_MLR/plots/general/cost/exp/'
output_plot_general_d_dir = './experiments/UCI_MLR/plots/general/distance/'
output_plot_general_d_n_dir = './experiments/UCI_MLR/plots/general/distance/norm/'
output_plot_general_d_e_dir = './experiments/UCI_MLR/plots/general/distance/exp/'

output_plot_chrc_dir = './experiments/UCI_MLR/plots/characteristics/'
output_plot_chrc_n_dir = './experiments/UCI_MLR/plots/characteristics/norm/' 
output_plot_chrc_e_dir = './experiments/UCI_MLR/plots/characteristics/exp/'

output_plot_metrics_dir = './experiments/UCI_MLR/plots/metrics/'
output_plot_metrics_n_dir = './experiments/UCI_MLR/plots/metrics/norm/'
output_plot_metrics_e_dir = './experiments/UCI_MLR/plots/metrics/exp/'
...
def plot_general_cost_boxplots (df, plot_metadata, ds_keys, cost_type):
    ...
    fig, _ = plt.subplots(nrow, ncol, sharey=True, sharex=True, figsize=(7, 8))
...
def plot_characteristics_pointplots (df, descriptions_df, plot_metadata, identifier, cost_type):
    ...
    if (cost_type=='Normalized'):
        cost_type_abr = 'Norm'
        save_dir = output_plot_chrc_n_dir
    elif (cost_type=='Expected'):
        cost_type_abr = 'Exp'
        save_dir = output_plot_chrc_e_dir

    
    
    subplot_row_titles = ['Dataset Size', 'No. of Features','Class Distribution']
    ...
    for subplot_row_title in subplot_row_titles:
    
        df[subplot_row_title] = 0
        
        for ds_key in descriptions_df['Dataset']:
            
            df.loc[df['Dataset']==ds_key, subplot_row_title] = float(descriptions_df.loc[descriptions_df['Dataset']==ds_key, subplot_row_title])
    ...
    fig, _ = plt.subplots(nrow, ncol, sharey=True, figsize = (6,5))
    ...
    for i, ax in enumerate(fig.axes):
        ...
        sns.lineplot(
            y='Cost Difference', 
            x=subplot_row_titles[r], 
            hue=subplot_row_titles[r],
            style=subplot_row_titles[r],
            hue_order= descriptions_df[subplot_row_titles[r]],
            style_order= descriptions_df[subplot_row_titles[r]], 
            markers=ds_markers,
            data=df_t,
        ...
        model = sm.OLS(df_t["Cost Difference"], sm.add_constant(df_t[subplot_row_titles[r]])).fit()

        # print(model.params)
        intercept, slope = round(model.params[0],2), round(model.params[1],2)
        r_squared, p_value = round(model.rsquared, 2), round(model.pvalues.loc[subplot_row_titles[r]], 2) 
        ax.text(
            0.7, 
            0.85, 
        ...
        if (r==0):
            ax.set_title('$\\frac{P_{test}(Y=1)}{P_{train}(Y=1)}$='+str(subplot_column_titles[c]), fontsize=7)
        else:
            ax.set(title=None)
    ...
    fig.suptitle(f'{cost_type} Cost Difference', y =0.9675, fontsize=8)
    fig.tight_layout()    

    fig.savefig(f'{save_dir}ds_cost_{identifier}.png', bbox_inches='tight', dpi=300)
...
def plot_results():
    ...
    if not os.path.exists(output_plot_chrc_dir):
        os.makedirs(output_plot_chrc_dir)

    if not os.path.exists(output_plot_chrc_n_dir):
        os.makedirs(output_plot_chrc_n_dir)

    if not os.path.exists(output_plot_chrc_e_dir):
        os.makedirs(output_plot_chrc_e_dir)
    ...
    dataset_descriptions = dataset_descriptions.rename(
        columns={
            'Non-missing Instances' : 'Dataset Size',
    ...
    for k in characteristics_plots_metadata:
        col_names = list(characteristics_plots_metadata[k].keys())

        current_slice_idx = True
        for j in col_names:
            current_slice_idx &=  (performance_df_summarized[j]==characteristics_plots_metadata[k][j])

        current_df = performance_df_summarized[current_slice_idx].copy()


        plot_characteristics_pointplots(current_df, dataset_descriptions.copy(), characteristics_plots_metadata[k], k, 'Normalized')
        
        plot_characteristics_pointplots(current_df, dataset_descriptions.copy(), characteristics_plots_metadata[k], k, 'Expected')
